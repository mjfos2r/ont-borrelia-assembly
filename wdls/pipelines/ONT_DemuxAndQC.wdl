...
workflow ONT_DemuxAndQC {
    ...
    parameter_meta {
        samplesheet: "samplesheet containing sample information, most importantly, the barcode_id and sample_alias. for singleplex runs, set barcode and sample_alias to the same value."
        ...
        singleplex: "boolean flag to indicate whether the run being processed is barcoded or singleplex. must specify sample_id if set to true"
        ...
        sample_id: "optional string specifying the sample_id for a singleplex run. must be provided for singleplex processing otherwise it will fail."
    }
    ...
    call GenUtils.DecompressRunTarball { input: tarball = RunTarball, is_valid = run_is_valid, singleplex = singleplex, sample_id = sample_id }

    call SSUtils.ParseSamplesheetToDataTable {
        input:
            samplesheet = samplesheet,
            file_paths = DecompressRunTarball.glob_paths,
            target_datatable = target_datatable,
    }

    output {
        # Metadata parsed from samplesheet, select whichever was executed.
        ...
        Array[File] bam_list = DecompressRunTarball.file_list
